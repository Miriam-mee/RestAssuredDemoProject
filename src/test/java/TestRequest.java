...
public class TestRequest {
    ...
    public void test_get_1(){
        ...
        given().param("page", 2).get("api/users").then()
                .body("data.id[1]", equalTo(8)).body("data.last_name",
                        hasItems("Lawson", "Ferguson", "Funke")).statusCode(200).log().all();
    }
...
@Test
    ...
    given().contentType(ContentType.JSON).body(jsonObject.toJSONString()).when().post(
"http://localhost:9191/productcatalog/products").then().statusCode(HttpStatus.SC_OK);
...
/*@Test
    public void test_post_2(){
        ...
        Map<String, Object> map = new HashMap<>();
        map.put("name", " tablet 2");
        map.put("quantity", Integer.valueOf(1));
        map.put("price", Integer.valueOf(300));
    System.out.println("Map :"  +map);
    JSONObject jsonObject = new JSONObject(map);
    System.out.println("jsonobject :" +jsonObject);
    given().header("jsonObject","Java").contentType(ContentType.JSON).
